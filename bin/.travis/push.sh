...
if [ "$1" == "" ]; then
    echo "Argument 1 variable REMOTE_IMAGE is not set, format: ezsystems/php. Bailling out !"
    exit 1
fi
if [ "$2" == "" ]; then
    echo "Argument 2 variable GIT_TAG is not set, format: v1.2.3. Bailling out !"
    exit 1
fi
