...
# Scan for environment variables prefixed with PHP_FPM_INI_ENV_ and inject those into /usr/local/etc/php-fpm.d/zzz_custom_settings.ini
# Environment variable names cannot contain dots, so use two underscores in that case:
# PHP_FPM_INI_ENV_pm__max_children=10  --> pm.max_children=10
if [ -f /usr/local/etc/php-fpm.d/zzz_custom_settings.ini ]; then rm /usr/local/etc/php-fpm.d/zzz_custom_settings.ini; fi
echo '[www]' > /usr/local/etc/php-fpm.d/zzz_custom_settings.ini
env | while IFS='=' read -r name value ; do
  if (echo $name|grep -E "^PHP_FPM_INI_ENV">/dev/null); then
    ...
    # Replace __ with .
    name=${name//__/.}
    echo $name=$value >> /usr/local/etc/php-fpm.d/zzz_custom_settings.ini
  fi
done
